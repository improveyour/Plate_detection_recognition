...
# 识别操作封装
def FinalRec(device, detect_model, plate_rec_model):
    img = cv_imread(opt.image_path)  # opencv 读取图片
...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if query_params.get("flag")[0] == "false":
            # 车辆出库的时候修改路径
            img_path = "C:\\Users\\Administrator\\Desktop\\download\\" + query_params.get("fileName")[0]
        else:
            # 车辆入库时使用的路径
            img_path = "E:\\graduate\\park-system\\ruoyi-admin\\src\\main\\resources\\plate_img\\" + query_params.get("fileName")[0]
        ...
        # 获取识别结果
        self.response_content = draw_result(img, dict_list)
        print(self.response_content)
        # img_name = os.path.basename(img_path)
        # # 移除
        # os.remove("E:\\graduate\\Chinese_license_plate_detection_recognition-main\\imgs\\test\\" + img_name)
...
if __name__ == '__main__':
    ...
    # plate_image = 'E:\\graduate\\park-system\\ruoyi-admin\\src\\main\\resources\\plate_img\\yueA636YK.png'
    ...
    #         if len(os.listdir(opt.image_path)) != 0:
    #             file_list = []
